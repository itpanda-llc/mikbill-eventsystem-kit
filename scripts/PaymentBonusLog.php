...
/** Размер бонуса (Процент от суммы платежа) */
const BONUS_PERCENT = 5;

/** Наименьший размер платежа для начисления бонуса (Денежная единица) */
const BONUS_START_AMOUNT = 2000;
...
function getClient(): ?array
{
    $sth = getConnect()->prepare("
        ...
        WHERE
            ...
            `bugh_plategi_stat`.`uid` IS NOT NULL
                AND
            `bugh_plategi_stat`.`summa` >= :bonusStartAmount
                AND
            (
                `bugh_plategi_stat`.`summa` * :bonusPercent / 100
            ) >=
            :bonusMinAmount
                AND
    ...
    $sth->bindValue(':bonusMaxAmount',
        BONUS_MAX_AMOUNT,
        PDO::PARAM_INT);
    $sth->bindValue(':bonusStartAmount',
        BONUS_START_AMOUNT,
        PDO::PARAM_INT);
